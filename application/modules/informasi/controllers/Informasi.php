...
class Informasi extends MX_Controller
{
    ...
    public function skpelayanan()
    {
        $data['title'] = 'Syarat dan Ketentuan Pelayanan';
        $data['user'] = $this->db->get_where('user', ['username' =>
        $this->session->userdata('username')])->row_array();

        $data['content'] = '';
        $page = 'informasi/skpelayanan';
        // echo modules::run('template/loadview', $data);
        echo modules::run('template/loadview', $data, $page);
    }
    ...
    {
        $data['title'] = 'Pelayanan Kesehatan Terdekat';
        $data['user'] = $this->db->get_where('user', ['username' =>
        $this->session->userdata('username')])->row_array();

        $data['content'] = '';
        $page = 'informasi/pelayanan_kesehatan';
        // echo modules::run('template/loadview', $data);
        echo modules::run('template/loadview', $data, $page);
    }
    ...
    public function tabelpelkes()
    {
        $fetch_data = $this->Informasi_model->make_datatables_pelkes();
        ...
        foreach ($fetch_data as $row) {
            ...
            $sub_array = array();
            $sub_array[] = $no;
            $sub_array[] = $row->nama_puskesmas . "<br>" . $row->nomor_telepon_puskesmas;
            $sub_array[] = $row->alamat_puskesmas . "<br>" . $row->kelurahan . ' ' . $row->kecamatan . ' ' . $row->kabupaten_kota;
            $sub_array[] = $row->nama_petugas_pemegang_program_jiwa . "<br>" . $row->no_hp;
        ...
        $output = array(
            "draw"                => intval($_POST['draw']),
            "recordsTotal"        => $this->Informasi_model->get_all_data_pelkes(),
            "recordsFiltered"     => $this->Informasi_model->get_filtered_data_pelkes(),
            "data"                => $data
        );
        echo json_encode($output);
    }
